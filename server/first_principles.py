...
def encounter_value(archetype, powers, ctx, totals, scenario="general", arch_row=None,
    ...
    if _hench_n:
        ...
        my_dps += sum((p.get("dps_total") or p.get("dps_each") or p.get("dps") or 0)
                      for p in (off.get("pets") or []))
    # v36: pet share measured BEFORE the common multipliers so the meter terms
    # can exempt it — inherent meters ride the PLAYER'S OWN attacks, never pets
    # (Containment must not double Singularity; Scourge is the Corruptor's roll).
    _pet_frac = 0.0 if my_dps <= 0 else max(0.0, min(1.0, 1.0 - (
        ((off.get("st_dps") or 0) * 0.4 + (off.get("aoe_dps") or 0)
         * min(sc["enemies"], 10) * 0.6)
        * (1.0 - 0.5 * sc.get("slow_in", 0.0) * (1.0 - min(slow_res, 1.0)))
        / my_dps)))
    ...
    # applied to the personal share only — the pet share passes through unchanged.
    _mm = _meter_damage_mult(archetype, sc, totals)
    my_dps *= (1.0 - _pet_frac) * _mm + _pet_frac
