...
SCENARIOS = {
    ...
    "fire_farm": dict(dmg_type="Fire", enemies=10, enemy_dps=110, teammates=0, team_dps=0,   length=20.0,  shift=4, rank_acc=1.00, ctrl_land=0.95, debuff_res=0.0,  enemy_regen=5,   def_debuff_in=0.05, kb_in=0.02, slow_in=0.0),
    ...
    # inspirations), +4 fire-typed pressure. v49: the window is an AFK STINT
    # (600s — ten minutes away from the keyboard, stated constant), not a 60s
    # spawn — a build whose passive sustain meets the stream holds
    # availability ~1 indefinitely; one that cannot dies early in the stint
    # and its availability collapses. That is how the AFK requirement enters
    # the objective through physics instead of a coded ban.
    "farm_afk":  dict(dmg_type="Fire", enemies=27, enemy_dps=110, teammates=0, team_dps=0,   length=600.0,  shift=4, rank_acc=1.00, ctrl_land=0.95, debuff_res=0.0,  enemy_regen=5,   def_debuff_in=0.05, kb_in=0.02, slow_in=0.0),
    ...
    "farm_active": dict(dmg_type="Fire", enemies=10, enemy_dps=110, teammates=0, team_dps=0, length=20.0,  shift=4, rank_acc=1.00, ctrl_land=0.95, debuff_res=0.0,  enemy_regen=5,   def_debuff_in=0.05, kb_in=0.02, slow_in=0.0, survival_is_constraint=True),
...
_PP_BELOW = {0: 1.00, 1: 0.90, 2: 0.80, 3: 0.65, 4: 0.48, 5: 0.30,
             # +6/+7 (v38): a −2 henchman in +4 content fights +6 — wiki
             # Purple_Patch table, fetched 2026-07-28 (docs/pet-tohit-sources.md)
             6: 0.15, 7: 0.08}
...
_PLAYER_BASE_VS = {-1: 0.80, 0: 0.75, 1: 0.65, 2: 0.56, 3: 0.48, 4: 0.39, 5: 0.30,
                   # +6/+7 (v38, for tier-shifted henchmen; wiki re-verified
                   # 2026-07-28 — the same Attack Mechanics table)
                   6: 0.20, 7: 0.08}
...
def _henchman_availability(pet, totals, sc, tohit_deb, bodies, owner_base_hp,
    ...
    squad — provisional, flagged); henchman innate defenses and MM aura buffs
    are not modeled (flagged; both UNDERSTATE availability). v38 note: the
    tier level shift is now modeled on the OFFENSE side (pet hit chance);
    this INCOMING side still ignores it — a −2 henchman is easier for the
    spawn to hit than modeled here, so T1 availability stays slightly
    overstated (flagged). The MM's own −tohit debuffs protect henchmen
    exactly as they protect him (game rule)."""
...
# ── v31 AFK sustain assessment (Joel's ruling, 2026-07-16; requirements made
# build-relative in v48) ─────────────────────────────────────────────────────
# When a combo cannot meet the AFK floor, the certificate does not relax the
# floor and does not hold certification — it states, from this model's own
# arithmetic, the difficulty tier the build DOES sustain.
# v48 (Maelwys round 4, topic 64761): the v31–v47 requirement ladder was ONE
# absolute (37 HP/s at +4x8 — Maelwys's number for a REFERENCE farmer at
# capped Fire resistance and softcapped defense) scaled only by the critter
# accuracy multiplier per shift. It never read the build in front of it, so a
# 63% Fire-res build out-tiered two res-capped ones on regen alone — the
# tiers anti-correlated with actual mitigation. The requirement is now THIS
# BUILD's own incoming stream in the farm_afk asteroid, computed with the
# same arithmetic and scenario constants the score's survival side already
# uses (v47 typed reading): position+Fire defense per incoming half,
# DDR-eroded, typed resistance capped at the game's 90%. Toggle −ToHit auras
# are honest upside deliberately NOT credited here (the score side credits
# them; the label must not claim a tier that depends on them — conservative,
# stated).
# The sustain ledger: regen + the SINGLE best self-heal power's sustained
# rate (heal-strength multiplied) — AFK play allows exactly ONE auto-fire
# power, so aggregating multiple click heals would certify a rate no absent
# player can click (caught on the very first stamp: Aid Self + Healing
...
def _afk_autofire_heal(powers, ctx, role_output_mod, attack_claims):
    """The ONE click heal an absent player can run (on auto-fire): best
    non-interruptible, non-rez self-heal — or nothing when the offense side
    claims the auto-fire slot (v46 one-auto-fire rule). ONE copy of the
    selection rule, shared by the certification ledger and the v49 score.
    Returns (power_leaf_name, raw_hps) before heal-strength scaling."""
    if not role_output_mod or attack_claims:
        return None, 0.0
    ...
    for p in (powers or []):
        ...
        if (self_hps <= 0 or is_rez
                or (rec.get("interrupt_time") or 0) > 0):
            continue
        if self_hps > best:
            best_name = (rec.get("full_name") or "").split(".")[-1]
            best = self_hps
...
def afk_sustain_assessment(powers, totals, arch_row, ctx, role_output_mod=None,
    ...
    # v46 ONE AUTO-FIRE, ONE LEDGER (Maelwys, topic 64761, round 2): the game
    # allows a single auto-fire power, and a real AFK farmer spends it on a
    # PBAoE attack — the v45 offense rule already claims it for the best click
    # attack. Crediting a click heal on auto-fire AT THE SAME TIME double-booked
    # the slot (the stated v45 limit, now closed). If the offense side claims
    # the slot, sustain is PASSIVE ONLY: regeneration plus toggle/auto heals.
    _attack_claims_autofire = bool((totals.get("offense") or {}).get("afk_autofire"))
    if role_output_mod:
        pbf = ctx.get("power_by_full") or {}
        for p in (powers or []):
            rec = pbf.get(p.get("full_name"))
        ...
        # the winner comes from the ONE shared selection rule (v49)
        _an, _ah = _afk_autofire_heal(powers, ctx, role_output_mod,
                                      _attack_claims_autofire)
        if _an:
            auto_name, auto_hps = _an, round(_ah * heal_str, 2)
    # v48 (Maelwys round 4): the ledger rows are DISPLAY — only the auto-fire
    # winner is summed into sustain. Say so on each row, so the certificate
    # cannot be read as crediting click heals it never counted.
    for h in heal_rates:
        h["counted"] = (h["power"] == auto_name and auto_hps > 0)
    sustain = regen_hps + auto_hps
    # v48 build-relative requirements (see the block comment above): this
    # build's own incoming HP/s per shift, same constants as the score side.
    sc = SCENARIOS["farm_afk"]
    _dt = sc.get("dmg_type") or "Smashing"
    _ddr = min((((totals.get("bonus_extras") or {}).get("def_debuff_resist")
                 or {}).get("value") or 0.0) / 100.0, 0.95)
    ddr_in = sc.get("def_debuff_in", 0.0) * (1.0 - _ddr)
    def_ml = max(_def_against(totals, ["Melee", _dt]) - ddr_in, 0.0)
    def_rn = max(_def_against(totals, ["AoE", _dt]) - ddr_in, 0.0)
    res_t = min((((totals.get("resistance") or {}).get(_dt) or {})
                 ).get("value", 0) or 0, 90.0) / 100.0
    reqs = {}
    for n in range(5):
        sc_n = dict(sc, shift=n)
        p_ml = incoming_hit(def_ml, 0.0, sc_n)
        p_rn = incoming_hit(def_rn, 0.0, sc_n)
        reqs[n] = round(sc["enemies"] * sc["enemy_dps"]
                        * (0.5 * p_ml + 0.5 * p_rn) * (1.0 - res_t), 1)
    tier = max((n for n in reqs if sustain >= reqs[n]), default=None)
    basis = (f"this build's {res_t * 100:.0f}% {_dt} resistance and "
             f"{def_ml * 100:.0f}%/{def_rn * 100:.0f}% melee/ranged defense")
    auto_part = (f" + {auto_name} on auto-fire {auto_hps:.1f}" if auto_name else "")
    if _attack_claims_autofire:
        auto_part += (" (auto-fire is spent on the attack — sustain is fully "
                      "passive, the way AFK farming actually runs)")
    if proc_regen_pct:
    ...
    if tier == 4:
        label = (f"AFK-certified at +4x8: {sustain:.1f} HP/s sustained "
                 f"(regen {regen_hps:.1f}{auto_part}) meets the {reqs[4]:.0f} HP/s "
                 f"the asteroid scrum deals through {basis}." + assume_part)
    elif tier is not None:
        label = (f"AFK-certified at +{tier}x8 (sustain {sustain:.1f} HP/s: regen "
                 f"{regen_hps:.1f}{auto_part} vs {reqs[tier]:.0f} taken there); "
                 f"+4x8 deals {reqs[4]:.0f} HP/s through {basis} — not "
                 f"sustained." + assume_part)
    else:
        label = (f"Does not sustain AFK play at any shift ({sustain:.1f} HP/s vs "
                 f"{reqs[0]:.0f} HP/s taken at +0x8 through {basis}) — active "
                 f"play only." + assume_part)
    return {"hp": round(hp, 1), "regen_hps": round(regen_hps, 2),
            ...
            "mitigation": {"dmg_type": _dt, "res_pct": round(res_t * 100, 1),
                           "def_melee_pct": round(def_ml * 100, 1),
                           "def_ranged_pct": round(def_rn * 100, 1)},
            "tier": tier, "label": label}


def _meter_damage_mult(archetype, sc, totals):
    """v36 INHERENT METER TERMS (Joel's ruled batch + basis audits, 2026-07-22).
    Multiplier on MY damage from archetype inherents whose basis passed the
    no-wiki audit. Additive +dmg families convert through (1+E+B+X)/(1+E+B)
    with E = standard post-ED attack enhancement (0.95, stated constant) and
    B = the build's credited global damage buff — additive with enhancement is
    the game's own +dmg arithmetic. NEGATIVE CONTROL BY CONSTRUCTION: every
    term keys off the archetype AND the build's own attack reality; meterless
    builds and other ATs read exactly 1.0.

    SCORING FLOOR (basis-audited, game-sourced only):
      Defiance (Blaster)   — DERIVED: each blast grants +dmg strength
                             0.066×cast ÷ areafactor for cast+7.5s (export
                             template). Nonstop-chain steady state = a rolling
                             (7.5+avg_cast)s window of casting → X = 0.066 ×
                             (7.5 + avg_cast) on the ST chain (area factor ≈ 1
                             for the ST rotation; stated simplification, same
                             assumption class as chain_end_per_sec). Zero
                             attacks → zero term.
      Vigilance (Defender) — DERIVED: +30% solo stepping −10% per teammate
                             (inherent scales 0.30/−0.1×3; scenario declares
                             teammates).
      Scourge (Corruptor)  — DERIVED from the export's OWN ramp requires
                             (chance 0 above 50% target HP, linear to 100% at
                             10%): damage-weighted expected extra over a
                             killed target = ∫ramp d(dmg) = 0.4/2 + 0.1 =
                             +30%; applies where fights end in kills (all
                             scenarios; stated).
      Containment (Controller) — grounded (paired equal-scale damage templates
                             on controller attacks); double weighted by the
                             scenario's OWN control-landing physics: mult =
                             1 + ctrl_land (mezzed-target fraction proxy —
                             ruled; AV's 0.08 says honestly you cannot
                             contain an AV).
    ...
    if archetype == "Class_Blaster":
        ...
        casts = [a.get("cast_time") for a in (off.get("attacks") or [])
                 if a.get("cast_time")]
        if casts and (off.get("st_dps") or 0) > 0:
            x = 0.066 * (7.5 + sum(casts) / len(casts))
    elif archetype == "Class_Defender":
        x = max(0.0, 0.30 - 0.10 * min(int(sc.get("teammates", 0)), 3))
    elif archetype == "Class_Corruptor":
        mult *= 1.30
    elif archetype == "Class_Controller":
        mult *= 1.0 + min(sc.get("ctrl_land", 0.0), 0.95)
    if x > 0:
        mult *= (1.0 + E + B + x) / (1.0 + E + B)
    return mult



# ── v43 DOMINATION ───────────────────────────────────────────────────────────
# The Dominator inherent, and BOTH halves of it come from the client rather
# than from a wiki or a guess.
#
# THE MAGNITUDE, which is what the duty cycle is built from:
#   `Inherent.Inherent.Domination` is a Click with recharge 200.0 whose
#   Set_Mode template runs for 90 seconds. So the floor uptime is 90/200 = 45%,
#   and global recharge shortens the 200 exactly as it shortens any click. At
#   enough recharge this reaches 1.0, which is perma-dom - the thing Dominator
#   players actually build for. The model reproduces it rather than assuming it.
#
# THE SIZE, which the game states in words and the data confirms:
#   the help says control powers "will typically last 50 percent longer", and
#   across EVERY encoded pair in the client - 41 of 41 - the Domination variant
#   of a mez row carries exactly 1.5x the base duration scale. Two independent
#   signals agreeing is this project's standard for acting on a rule.
#
# ⚠⚠ APPLIED UNIVERSALLY, NOT PER ENCODED POWER. The client writes the variant
# rows out on only 12 of the 26 Dominator sets: Plant's Strangler has one and
# Mind's Dominate - the same tier-1 hold, same scale 12.0 mag 3.0 - does not.
# That is an ENCODING asymmetry, not a game one; the help says "your control
# powers" without qualification. Pricing only the encoded 12 would bias the
# solver toward those sets, which is precisely the "patch just the reported
# case" that the universal-rules doctrine forbids.
#
# ⚠ THE MAGNITUDE HALF IS DELIBERATELY NOT CREDITED. The help also says
# Domination lets you "more easily Dominate stronger opponents", and the
# variant rows do carry their own magnitude - but whether that magnitude ADDS
# to the base (both rows say stack: Stack, which would double a mag-3 hold to
# 6) or REPLACES it is genuinely ambiguous in the client, and three of the 41
# pairs do not fit either reading. A doubling of control magnitude across an
# archetype is too big to infer, so it is left out and stated. The term is an
# understatement, the same direction as every other honest gap here.
_DOM_DURATION_BONUS = 0.5      # "50 percent longer", client help + 41/41 rows
_DOM_WINDOW = 90.0             # seconds, the Set_Mode template
_DOM_RECHARGE = 200.0          # seconds, the inherent's own recharge
_DOM_SETS = ("Dominator_Control.", "Dominator_Assault.")


def _domination_duration_bonus(powers, global_recharge):
    """Extra mez DURATION fraction from Domination, or 0.0 for anyone else.

    ⚠ Detected from the build's own powersets rather than an archetype string,
    because `powers` is what this scope has and a Dominator is exactly someone
    holding Dominator_Control / Dominator_Assault powers. Domination is granted
    automatically, so holding the sets is holding the inherent.
    """
    for p in powers or []:
        ps = (p.get("powerset_full_name")
              or (p.get("full_name") or "").rsplit(".", 1)[0])
        if ps.startswith(_DOM_SETS):
            break
    else:
        return 0.0
    eff = _DOM_RECHARGE / (1.0 + max(0.0, global_recharge or 0.0))
    uptime = min(1.0, _DOM_WINDOW / eff) if eff > 0 else 1.0
    return _DOM_DURATION_BONUS * uptime
...
def encounter_value(archetype, powers, ctx, totals, scenario="general", arch_row=None,
    ...
    # v41: and the build's OWN defence-debuff resistance cuts that pressure. The
    # comment above ("a squishy has ZERO") described the only build the model
    # could see: DDR is power-granted, the game ships no DDR set bonus, and our
    # data carried none of it until the 178-power back-fill. Super Reflexes,
    # whose entire identity is that the haircut does not land on it, took the
    # same haircut as a Blaster. Capped at the game's own 95%.
    # ⚠ READ IT FROM bonus_extras - calculate_build's response is curated and
    # def_debuff_resist is not at the top level (the trap that cost a revert).
    _ddr = min((((totals.get("bonus_extras") or {}).get("def_debuff_resist")
                 or {}).get("value") or 0.0) / 100.0, 0.95)
    ddr_in = sc.get("def_debuff_in", 0.0) * (1.0 - _ddr)
    # v47 TYPED SCENARIOS (Maelwys's example builds exposed it): a scenario may
    # declare its incoming damage TYPE. Homecoming fire farms deal essentially
    # Fire damage — that is WHY the community farm doctrine caps Fire res/def —
    # yet this block read Smashing res and Smash/Lethal defense for every
    # scenario, so a 90% Fire-capped farmer measured a 6.6-second time-to-live
    # in its own farm (his RadM/FA: availability 0.102 vs the Stone build's
    # 0.839, the whole 20x contribution gap). With dmg_type set, defense reads
    # position+type against it and resistance reads that type; untyped
    # scenarios are byte-identical to v46.
    _dt = sc.get("dmg_type")
    if _dt:
        def_ml = max(_def_against(totals, ["Melee", _dt]) - ddr_in, 0.0)
        def_rn = max(_def_against(totals, ["AoE", _dt]) - ddr_in, 0.0)
    ...
    res_sl = ((((totals.get("resistance") or {}).get(_dt or "Smashing") or {})
               ).get("value", 0) or 0) / 100.0
    ...
    # v49 AFK SURVIVAL IS PASSIVE-ONLY (Joel's ruling, 2026-08-17 — the AFK
    # tier joins the objective): an absent player clicks nothing, so farm_afk
    # survival credits regeneration and toggles/autos (already in totals) plus
    # the ONE auto-fire heal — and none at all when the offense side claims
    # the slot. The v48 label and this score now judge the same build the
    # same way; every other scenario keeps the full click-heal aggregate.
    if scenario == "farm_afk":
        _an, _ah = _afk_autofire_heal(powers, ctx, ro,
                                      bool(off.get("afk_autofire")))
        self_heal_hps = _ah
    ...
    # v42 ABSORB. A shield of hit points soaks damage before your own HP does,
    # so as sustain it is damage-not-taken per second - the same units as a heal,
    # and it lands in the same place. The engine has already divided each pool by
    # how often its power re-arms (own recharge slotting included), because a
    # shield soaks its pool ONCE PER CAST: crediting the pool itself would score
    # a 120-second click as though it were permanently up.
    # ⚠ THREE HONEST UNDERSTATEMENTS, all in the same direction:
    #   * absorb does not overheal. A heal at full health is wasted and a shield
    #     is not, so treating them identically prices absorb slightly low.
    #   * the BURST value is not captured - a shield eats an alpha strike, and
    #     time-to-live arithmetic cannot see that.
    #   * the v29 heal-strength set bonuses are NOT applied to it. They multiply
    #     healing the build puts out; whether the game's Heal strength also
    #     scales absorb is not settled from the client, so it is left off.
    absorb_hps = ((totals.get("bonus_extras") or {}).get("absorb_hps") or {}).get("value") or 0.0
    net_in = max(incoming - regen_hps - self_heal_hps - absorb_hps, 1.0)
    ...
    # ── MEZ PROTECTION - protection-dominant, from a MEASURED distribution ──
    # (INERT, and therefore carries NO model version: see the mez_in note below.)
    ...
    # v45 AFK OFFENSE: farm_afk reads the engine's AFK aggregates — damage auras
    # (toggles/autos) plus exactly ONE click on auto-fire; the rest of the click
    # chain is damage nobody is there to cast. Same 0.4/0.6 combination as the
    # general case so scores stay comparable across contents.
    if scenario == "farm_afk":
        my_dps = ((off.get("afk_st_dps") or 0) * 0.4
                  + (off.get("afk_aoe_dps") or 0) * min(sc["enemies"], 10) * 0.6)
    else:
    ...
    # ── v38 PET DPS, own physics (wiki-sourced: docs/pet-tohit-sources.md) ──
    # Pets no longer ride the PLAYER's hit chance and purple patch (the silent
    # v10..v37 arrangement — "always hitting" on the display, the player's
    # p_out×pp in the score). Each pet now earns:
    #   hit  = clamp(acc_mult × clamp(base[shift + level_shift] + pet ToHit))
    #          — the PLAYER base-hit table (pets roll like players, rank acc
    #          1.0), at the PET's own level: henchman tiers fight
    #          sc.shift + 2/1/0, Levelminus-template pets +1.
    #   pp   = purple patch at the pet's own shift (T1 vs +4 farm = +6 → 0.15).
    # acc_mult = the attacks' client inherent accuracy × summon-slotted
    # Accuracy (copy_boosts-gated, ED); ToHit arrives via the same
    # buff_effects lever as pet damage buffs (Supremacy +10%, Tactics — a
    # self_effects-only ToHit like Focused Accuracy never routes). Buffs are
    # unresistible: no purple patch on the ToHit itself (team-buff precedent).
    # Pets stay exempt from the player's hasten chain elasticity and from
    # inherent meters (v36 exemption, now structural); end_factor still
    # multiplies the whole (stated conservative: an end-broken MM keeps
    # commanding standing henchmen — unmodeled upside).
    # v29 henchman availability (renewal) unchanged and still multiplies.
    _pets = off.get("pets") or []
    _pet_th_all = (off.get("pet_tohit_all_pct") or 0.0) / 100.0
    _pet_th_top = (off.get("pet_tohit_top_pct") or 0.0) / 100.0

    def _pet_out(pet, i):
        shift = sc.get("shift", 0) + (pet.get("level_shift") or 0)
        base = _PLAYER_BASE_VS.get(shift, 0.08 if shift > 7 else 0.39)
        th = _pet_th_all + (_pet_th_top if i == 0 else 0.0)
        hit = _clamp((pet.get("acc_mult") or 1.0) * _clamp(base + th))
        return hit * _PP_BELOW.get(shift, 0.01)

    _hench_n = sum((p.get("count") or 1) for p in _pets
    ...
    if _hench_n:
        ...
        pet_dps = sum((p.get("dps_total") or p.get("dps_each") or p.get("dps") or 0)
                      * _henchman_availability(p, totals, sc, tohit_deb,
                                               _bodies, base_hp, _up_cast)
                      * _pet_out(p, i)
                      for i, p in enumerate(_pets))
    else:
        pet_dps = sum((p.get("dps_total") or p.get("dps_each") or p.get("dps") or 0)
                      * _pet_out(p, i)
                      for i, p in enumerate(_pets))
    my_dps *= p_out * pp * hasten_mult_dmg   # hit chance × purple patch × click-recharge credit
    # v36: archetype inherent meter terms (basis-audited only; see _meter_damage_mult),
    # personal share only — pets are structurally outside my_dps now (v38).
    _mm = _meter_damage_mult(archetype, sc, totals)
    my_dps *= _mm
    my_dps += pet_dps
    ...
    # v36 DOMINATION (perma-derived, ruled): perma condition from the build's
    # own recharge (base 200s vs 90s bar → perma at ≥ +122% incl. Hasten avg);
    # a perma bar = a full 100-end refill every ~90s = +1.11 end/s recovery-
    # equivalent, STATED on labels. Mez-magnitude double stays DORMANT
    # (ctrl_land is a mag-3 scenario constant — "shown, not yet scored").
    _dom_refill = 0.0
    if archetype == "Class_Dominator" and (passive_rech + h_avg) >= 1.22:
        _dom_refill = 100.0 / 90.0
    if endb.get("travel_in_combat"):
        d_tog += endb.get("travel_toggle_drain_per_sec") or 0.0
    drain = d_chain + d_tog
    rec_ps = (endb.get("recovery_per_sec") or 0.0) + _dom_refill
    ...
    if ro:
        ...
        # v43 DOMINATION. The Dominator inherent, and the game states it in its
        # own help: "When your Domination bar is over 90%, you can activate this
        # power ... Your control powers will typically last 50 percent longer".
        # A duration bonus is exactly what mez_dur carries, so it rides the
        # channel the v30 set bonuses already use - no second mechanism.
        _dom = _domination_duration_bonus(powers, passive_rech + h_avg)
        if _dom:
            for _m in ro.CONTROL_WEIGHT:
                _mzd[_m] = _mzd.get(_m, 0.0) + _dom
