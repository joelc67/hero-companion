...
- **CHAMPION #29 CERTIFIED (2026-08-18, Joel's gaming PC).** `Class_Corruptor|Corruptor_Ranged.Fire_Blast|Corruptor_Buff.Radiation_Emission|itrial` gold-standard solved on the gaming-PC mirror (i9-9900K, `C:\Users\Joel Chambers\hero-companion`, Python 3.14): converged, 6 restarts, not truncated, run score 1329.92 (eps/lex/plain unanimous), canonical_score 1322.77 via evaluate_first --write against model 49. The same evaluate-first pass reproduced all 28 prior canonicals EXACTLY (0 moved, 0 failed) — the gaming PC is a proven-faithful scoring environment. ⚠ The pass also confirmed the roster-wide stale stamps (26 of 29 at v44/v45 < v49) — a recert wave is owed when Joel schedules it. (Same-session note: an aider+qwen3-coder:30b local-LLM attempt at this combo failed champion-grade validation — hallucinated power name, role-inverted picks — reconfirming "champions are data, NOT AI".)
...
- Release procedure: **data-currency check first** (`tools/reality_check_liveness.py` + confirm the newest LIVE patch on the forum's patch-notes page) → bump VERSION → **smoke pins** (⚠ smoke_release has TWO version pins — port-race guard AND assertion; smoke_gold same + model pin) → CHANGELOG date → `python tools\build_help_pdf.py` → **commit/push the prep** → stop HeroCompanion processes → PyInstaller `HeroCompanion.spec --noconfirm` → copy "Add Shortcuts.bat" into `dist\HeroCompanion\` → frozen-exe smoke + gold → sign exe → ISCC (`%LOCALAPPDATA%\Programs\Inno Setup 6\ISCC.exe` — per-user, NOT Program Files; reads VERSION dynamically) → sign installer → Compress-Archive zip → **publish via the GitHub API** with BOTH assets → verify via the API **and** that `/meta/update-check` returns the new version.
- ⚠ **COMMIT THE PREP BEFORE BUILDING** (2026-09-19 — this REVERSES the old order, which built first). A frozen build stamps `git describe --dirty`, so building while VERSION/CHANGELOG are still uncommitted bakes `<sha>-dirty` into the shipped exe: permanent ambiguity about what source made a signed artifact. 0.12.49 shipped that way and cost a full rebuild + both smokes again + re-sign to correct; 0.12.50 committed first and stamped `a6d3e39` clean on the first build.
- ⚠ **`gh release create` DOES NOT WORK ON THIS BOX.** The global `.gitconfig` routes github.com through `gh auth git-credential`, and gh's active account is `bulkdelete` (the extension family's), so joelc67 repos 403 — this is a wrong-identity symptom, NOT an expired token, so do not re-auth and do not `gh auth switch`. Publish via the API with the GCM joelc67 token (`git credential fill` fed through a temp stdin FILE; PS5.1 pipes garble it), the way 0.12.47–0.12.50 went out. ⚠ `target_commitish` must be the **FULL 40-char SHA** — an abbreviated one 422s with the misleading pair "tag_name is not a valid tag" + "invalid target_commitish", and creates nothing. Same root cause makes `git push` 403 until the repo carries a local `credential.https://github.com` override (empty helper, then `manager`, username joelc67).
- ⚠ **Let each frozen exe exit before launching the next.** Back-to-back smokes hit the single-instance defer: a gold run started while the release smoke's instance still owned 5000 dies instantly with `ConnectionResetError` before one context runs. Not a build failure — re-run it.
- The repo is public on GitHub (github.com/joelc67/hero-companion; Joel's GitHub = joelc67; ⚠ the gh CLI here is authed as `bulkdelete`/`bulkdeletetools`, NOT joelc67 — see the publish warning above). Keep raw brainstorms and personal notes out of commits. Ideation notes live in `C:\Users\joelc\code\ideas.md` (outside this repo); when Joel says "read ideas.md", that's the file.
- **Push discipline:** the Pulse pipeline pushes to this repo's master ~30×/day, so a rejection on push is routine, not a conflict. On rejection, **rebase your own unpushed commits onto origin/master** (`git fetch origin && git rebase origin/master`) and push — that keeps the history linear instead of threading a merge bubble through every Pulse commit. (Joel's ruling 2026-09-20, relaxing the old "merge, NEVER rebase": the rule was about not rewriting PUBLISHED history, and local-only commits are not published. Quote hashes from AFTER the rebase — that is what lands on origin. Never rebase or force-push anything already on origin.)
